...
def add_dit_training_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        "When greater than zero, pre-generates shuffled/dropout variants and selects one by epoch. "
        "Requires disk caching"
        " / テキストエンコーダ出力のキャプションバリエーション数。0より大きい場合、"
        "シャッフルやタグdropout済みの複数キャッシュを生成してepochごとに切り替えます。",
    )
...
def add_dataset_arguments(
    ...
    parser.add_argument(
        ...
        default="tags",
        choices=["tags", "mixed"],
        help="caption mode: tags (default) or mixed composition of tag and natural-language captions"
        " / キャプションモード: tags（既定）またはタグと自然言語を組み合わせるmixed",
    )
    parser.add_argument(
        "--protected_tags_file",
        type=str,
        default=None,
        help="path to a text file containing tags that caption tag dropout must preserve"
        " / caption tag dropout에서 제외할 태그가 들어 있는 텍스트 파일 경로",
    )
    parser.add_argument(
        "--caption_extention",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="log sampled captions every N steps (0 disables caption logging)"
        " / Nステップごとに処理後のキャプションをログへ出力します（0で無効）",
    )
    parser.add_argument(
        "--log_captions_max_length",
        type=int,
        default=20,
        help="maximum number of caption tokens shown in caption debug logs"
        " / キャプションデバッグログに表示する最大トークン数",
    )
    parser.add_argument(
        "--resolution",
        type=str,
    ...
    parser.add_argument(
        "--bucket_no_upscale",
        action="store_true",
        help="make native-size buckets only for images smaller than the training resolution; larger images use predefined buckets"
        " / 学習解像度より小さい画像だけを拡大せず、より大きい画像は定義済みbucketを使用します",
    )
    parser.add_argument(
    ...
    if support_caption_dropout:
        ...
        parser.add_argument(
            ...
            type=float,
            default=0.0,
            help="dropout rate for fixed tokens selected by keep_tokens_separator or keep_tokens. "
            "Applied independently from caption_tag_dropout_rate; fixed tokens remain unshuffled"
            " / keep_tokens_separatorまたはkeep_tokensで固定したタグをdropoutする割合。"
            "caption_tag_dropout_rateとは独立に適用され、固定部分はシャッフルされません。",
        )
