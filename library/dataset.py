...
class BucketManager:
    ...
    def select_bucket(self, image_width, image_height):
        ...
        # Keep truly small images at their native size, but downscale images at
        # or above the target area into the configured predefined buckets.
        use_no_upscale = (
            self.no_upscale
...
class BaseDataset(torch.utils.data.Dataset):
        ...
        if (
            ...
            if (
                ...
                if subset.keep_tokens_separator in flex_part:
                    ...
                    fixed_suffix_tokens = [
                        token.strip()
                        for token in fixed_suffix_part.split(subset.caption_separator)
                        if token.strip()
                    ]

                fixed_tokens = [
                    token.strip() for token in fixed_part.split(subset.caption_separator) if token.strip()
                ]
                flex_tokens = [
                    token.strip() for token in flex_part.split(subset.caption_separator) if token.strip()
                ]
            else:
                tokens = [token.strip() for token in caption.strip().split(subset.caption_separator)]
                flex_tokens = tokens[:]
            ...
            if special_dropout_rate > 0 and (fixed_tokens or fixed_suffix_tokens):
                ...
                fixed_tokens = drop_fixed_tokens(fixed_tokens)
                fixed_suffix_tokens = drop_fixed_tokens(fixed_suffix_tokens)
            ...
            # The separator is control syntax and must never reach the text encoder.
            parts = caption.split(subset.keep_tokens_separator)
            all_tokens = []
            for part in parts:
                all_tokens.extend(
                    token.strip() for token in part.split(subset.caption_separator) if token.strip()
                )
            caption = f"{subset.caption_separator} ".join(all_tokens)
    ...
    def process_caption(
        ...
        subset: BaseSubset,
        caption: str,
        caption_nl: Optional[str] = None,
        skip_caption_dropout: bool = False,
    ):
            ...
            if subset.caption_mode == "mixed" and caption_nl:
                ...
                if special_dropout_rate > 0:
                    ...
                    if subset.keep_tokens_separator and subset.keep_tokens_separator in caption:
                        fixed_prefix = [
                            token.strip()
                            for token in caption.split(subset.keep_tokens_separator, 1)[0].split(
                ...
                if selected_mode == "tags":
                    ...
                    caption = ", ".join([part for part in fixed_prefix + [nl_caption] if part])
                elif selected_mode == "tags_nl":
                    caption = ", ".join([part for part in [tag_caption, nl_caption] if part])
                else:
                    if fixed_prefix:
                        rest_tags = [
                            part.strip()
                            for part in tag_caption.split(subset.caption_separator)
                            if part.strip()
                        ][len(fixed_prefix) :]
                        caption = ", ".join(fixed_prefix + [nl_caption] + rest_tags)
                    else:
                        caption = ", ".join([part for part in [nl_caption, tag_caption] if part])
    ...
    def make_buckets(self):
        ...
        if self.enable_bucket:
            if self.bucket_manager is None:  # fine tuningの場合でmetadataに定義がある場合は、すでに初期化済み
                ...
                # Large images still use the predefined buckets even when
                # bucket_no_upscale is enabled for smaller images.
            ...
            for info in batch:
                ...
                if (
                    ...
                    for part in parts:
                        all_tokens.extend(
                            token.strip()
                            for token in part.split(subset.caption_separator)
                            if token.strip()
                        )
                    info.caption = f"{subset.caption_separator} ".join(all_tokens)
    ...
    def new_cache_text_encoder_outputs_variants(
        ...
        for variant_idx in range(num_variants):
            ...
            logger.info(
                f"  caching {sum(len(batch_infos) for batch_infos, _ in batches)} "
                f"images for variant {variant_idx}..."
            )
    ...
    def __getitem__(self, index):
        ...
        for image_key in bucket[image_index : image_index + bucket_batch_size]:
            ...
            if image_info.text_encoder_outputs is not None:
                ...
                # on disk: select this epoch's pre-generated caption variant when enabled
                if (
            ...
            if tokenization_required:
                caption, caption_debug = self.process_caption(
                    subset, image_info.caption, image_info.caption_nl
                )
                ...
                # Cached embeddings still need a processed caption for diagnostics.
                caption, caption_debug = self.process_caption(
                    subset, image_info.caption, image_info.caption_nl
                )
